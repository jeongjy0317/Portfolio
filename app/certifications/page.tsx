    ...
    <div className="min-h-screen bg-paper">
      <main className="mx-auto max-w-[900px] px-5 pt-16 pb-40 md:px-14">
       <SwapIn>
        ...
        <Stagger>
          {certsQual.map((c, i) => (
            <StaggerItem key={c.id}><CertArticle cert={c} headingAs="h3" last={i === certsQual.length - 1} href={detailHref("certifications", c.id)} /></StaggerItem>
          ))}
        </Stagger>
        ...
        <Stagger>
          {certsEtc.map((c, i) => (
            <StaggerItem key={c.id}><CertArticle cert={c} headingAs="h3" last={i === certsEtc.length - 1} href={detailHref("certifications", c.id)} /></StaggerItem>
          ))}
        </Stagger>
       </SwapIn>
      </main>
